...
// 		this._activeIndictors(index);

// 		this.curIndex = index;
// 	};

// 	Slider.prototype._activeIndictors = function(index) {
		...
		if(this.options.loop && direction) {
			if(direction<0){ // click right
				...
				if(index === this.itemNum - 1) {
					this.$container.removeClass(this.transitionClass).css('left',-1*index*self.itemWidth);
